<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <a href = '<%=url%>'>Go Back</a>
	...
	<body>
		...
		function yourtic(name){
			if(name=="null"){
				alert("Select an user first!!");
			}
			else{
			var newurl = "http://localhost:8080/Web_Train/Yourtickets.jsp?name="+name ;
			window.location.replace(newurl);
		}
	}
		function booktic(name){
			if(name=="null"){
				alert("Select an user first!!");
			}
			else{
...
%>
 ...
 <div class="sidenav">
	 <ul>
		...
		%>
		<p1><a target="_blank" onclick="useropt('<%=name%>')" >
	<%
	if(!name.equals("JSESSIONID") && !name.equals("currentuser")) {
		out.println( name ); 
		cname = name;
	...
	<div class="main">
		...
		<div class="hd">
			<p1 class="menu"><a onclick="booktic('<%=c%>')">Book Your Ticket</a></p1>
		</div>
		<div class="hd">
			<p1 class="menu"><a onclick="availabletrains('<%=c%>')">Available Trains</a></p1>
		</div>
		<div class="hd">
			<p1 class="menu"><a onclick="yourtic('<%=c%>')">Your Tickets</a></p1>
		</div>	
		<div class="hd">
